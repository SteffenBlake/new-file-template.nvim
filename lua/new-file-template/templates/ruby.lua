...
local function model_template(path, filename, callback)
    local class_text = inheritance_class(filename, "ApplicationRecord")

    callback(generate_module_structure(class_text, 2, path))
end

local function business_template(path, filename, callback)
    local class_text = inheritance_class(filename, "ApplicationBusiness")

    callback(generate_module_structure(class_text, 2, path))
end

local function controller_template(path, filename, callback)
    local class_text = inheritance_class(filename, "ApplicationController")

    callback(generate_module_structure(class_text, 2, path))
end

local function mailer_template(path, filename, callback)
    local class_text = inheritance_class(filename, "ApplicationMailer")

    callback(generate_module_structure(class_text, 2, path))
end

local function generic_app_class(path, filename, callback)
    local text = [[
class ]] .. get_class_name(filename) .. [[|cursor|

end]]

    callback(generate_module_structure(text, 2, path))
end

local function lib_template(path, filename, callback)
    local text = [[
class ]] .. get_class_name(filename) .. [[
  ...
  |cursor|
    ...
    callback(generate_module_structure(text, 1, path))
end

local function jobs_template(path, filename, callback)
    local class_text = [[
...
local function task_template(_, filename, callback)
...
local function factory_template(_, filename, callback)
    local file = vim.split(filename, "%.")[1]
...
local function spec_template(relative_path, filename, callback)
...
local function avo_action_template(path, filename, callback)
    local class_text = [[
class ]] .. get_class_name(filename) .. " < Avo::BaseAction" .. [[
  ...
  self.name = "|cursor|"
    ...
    callback(generate_module_structure(class_text, 3, path))
end

local function avo_card_template(path, filename, callback)
    local class_text = [[
...
local function avo_resource_tool_template(path, filename, callback)
    local class_text = [[
